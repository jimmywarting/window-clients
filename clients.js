(() => {
  ...
  bc.onmessage = evt => {
    ...
    if (type === 'install') {
      idMap[id] = data
      bc.postMessage([ID, 'stateChange', idMap[ID]])
    }
  ...
  function handleStateChange() {
    ...
    installed && bc.postMessage([ID, 'stateChange', idMap[ID]])
  }
  ...
  bc.postMessage([ID, 'install', idMap[ID]])
  ...
  window.clients = new class Clients {
    ...
    matchAll(opts = {}) {
      return Promise.resolve(
        Object.keys(idMap)
        .filter(id => opts.includeSelf || id !== ID) // exclude self
